...
//   },
//   body: JSON.stringify({
//     from: "onboarding@resend.dev", // Resend's required testing sender
//     to: user.email, 
...
// Support Routes - Updated to Resend fetch API
app.post("/api/support", async (req, res) => {
  const { name, email, message, source } = req.body;
  ...
  try {
    ...
    // Using Resend fetch to send the support email
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: process.env.EMAIL_USER, // Sending the ticket to yourself
        reply_to: email, // If you reply to the email, it goes to the user
        subject: `New ${formSource} from ${name || "User"}`,
        text: `You received a new submission from your dashboard (${source || "Website"}):\n\nSender Name: ${name}\nSender Email: ${email}\n\nMessage:\n${message}`
      })
    });
